...
$("#add-button").click(function () {
    ...
    input.trim();
    if (input.length) {
        var inStackEl = leftInStack(input); // returns node of added element in right list of bought 
...
function addItem(title, inStackEl) {
    ...
    // node.find(".name").click( changeName(node, inStackEl ));
    node.find(".name").click( function changeName() {
        var name = node.find(".name").text();
        node.find(".name").css("display",	"none");
        node.find("#edit").val(name);
        node.find("#edit").css("display","unset");
        node.find("#edit").focus();
        node.find("#edit").blur(function updateName() {
            if(node.find("#edit").val() != "") {
               name =  node.find("#edit").val();
            }
            node.find("#edit").css("display",	"none");
            node.find(".name").text(name);
            inStackEl.find(".name-of-added").text(name); 
            node.find(".name").css("display",	"unset");
        })
    });
    node.hide();
    LIST.append(node);
...
// function changeName(node, inStackEl) {
//     var name = node.find(".name").text();
//     node.find(".name").css("display",	"none");
//     node.find("#edit").val(name);
//     node.find("#edit").css("display","unset");
